...
<body>
		<form:form id="pageListForm" action="list.html" commandName="baseQuery" method="get">
				...
				<dl class="queryPanel" >
					<c:forEach items="${baseQuery.items}" var="item" varStatus="index">
						...
						<dd>
							...
							<c:if test="${item.type=='String' or item.type=='Number'}">
								<input type="text" name="items[${index.index}].value" value="${item.value}"/>
							</c:if>
							<c:if test="${item.type=='Date'}">
								<form:input path="items[${index.index}].value" class="Wdate" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false,readOnly:true,disabled:true})"/>
							</c:if>
							<c:if test="${item.type=='Enum'}">
								<form:select path="items[${index.index}].value" items="${item.enumRanges}"   value="${item.value}" />
							</c:if>
							<c:if test="${item.type=='Boolean'}">
								<form:checkbox path="items[${index.index}].value" value="${item.value}"/>
							</c:if>
					...
					<dd><input type="button" onclick="gotoPage(1)" value="查询"/></dd>
					<dd><input type="button" onclick="toList()" value="重置"/></dd>
				</dl>
			</form:form>
</body>