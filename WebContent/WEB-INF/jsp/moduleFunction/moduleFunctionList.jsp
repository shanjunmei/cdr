		...
		<table class="pageList">
			...
			<c:forEach items="${dataList}" var="_data" varStatus="st">
				<c:if test="${st.index%2 eq 0}">
					<tr class="obb" onclick="rowSelect(this)"
						ondblclick="view('${_data.id}')">
				</c:if>
				...
				<td><fmt:formatDate value="${_data.lastModifyTime }"
						pattern="yyyy-MM-dd HH:mm" /></td>
				<td class="listOperation"><a href="#"
					onclick="edit('${_data.id}')">编辑</a> <c:if
						test="${'D' eq _data.enabled}">
						<a href="#" onclick="disableOrEnable('${_data.id}')">启用</a>
					</c:if> <c:if test="${'E' eq _data.enabled}">
						<a href="#" onclick="disableOrEnable('${_data.id}')">禁用</a>
					</c:if> <a href="#" onclick="deleteRow('${_data.id}')">删除</a> <a href="#"
					onclick="view('${_data.id}')">查看</a></td>
				</tr>
