...
public class UserController extends BaseController<User, String> {
	...
	@Override
	protected User prepare4save(User intance,HttpSession session) {
		intance=super.prepare4save(intance,session);
	...
	public Object logout(@PathVariable("ext") String ext,HttpSession session) {
		ModelAndView view=new ModelAndView();
		User user =getCurrentUser(session);
		if(user!=null){
			SysContextUtils.logout(session);
			if(RequestSuffixType.iv.name().equals(ext)){
				view.addObject("code", "1");
				view.addObject("msg", "success");
			}
